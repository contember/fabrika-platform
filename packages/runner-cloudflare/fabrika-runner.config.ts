// vozka-runner's OWN deploy surface — the deploy EXECUTOR, split out of the fabrika control plane so a
// deploy of fabrika never resets the container running that deploy (see src/worker.ts for the why).
//
// vozka-runner is INFRA, not a registered app: no Cloudflare Access front door (it's reachable only via
// fabrika's `RUNNER_SVC` service binding, never publicly), no propustka access/schema, no runtime secrets
// (every credential arrives per-run in the provider job over the binding). It is deployed RARELY and
// OUT-OF-BAND (scripts/bootstrap-runner.ts) — it can't deploy itself through itself (same self-reset),
// and it only changes when the relay / container / runner image changes.
//
// SHARED RESOURCES: RUN_LOGS (R2) + DB (D1) are the SAME bucket + database the control plane owns. oblaka
// ADOPTS an existing resource by its remote name (`<env>-<name>`), so declaring the same names here binds
// fabrika's existing resources rather than creating new ones. fabrika owns the schema + migrations; this
// worker only WRITES (run logs/status → R2, the runs row's terminal status → D1) — hence NO migrationsDir.
//
// Local dev still uses oblaka directly via the `oblaka.ts` shim, which calls `buildRunnerWorker` below.

import { Container, D1Database, defineApp, R2Bucket, type ResourceContext, Worker } from '@fabrika/provider-cloudflare'

/** Container instance type per stage — dev locally, larger off-local; any other env → basic. */
const instanceTypeFor = (env: string): 'dev' | 'basic' | 'standard' => {
	if (env === 'local') {
		return 'dev'
	}
	if (env === 'prod') {
		return 'standard'
	}
	return 'basic'
}

/**
 * Build vozka-runner's Cloudflare resource graph for one environment. The SINGLE source of the graph —
 * both the provider deploy/bootstrap path (via `defineApp` below) and the local-dev `oblaka.ts` shim
 * call this, so the two never drift.
 */
export const buildRunnerWorker = (ctx: ResourceContext): Worker => {
	const { env } = ctx
	const instanceType = instanceTypeFor(env)

	return new Worker({
		dir: '.',
		name: 'vozka-runner',
		main: './src/worker.ts',
		compatibility_flags: ['nodejs_compat'],
		compatibility_date: '2025-05-25',
		// No public route — vozka-runner is reachable ONLY via fabrika's RUNNER_SVC service binding.
		routes: [],
		observability: { enabled: true },
		vars: {
			ENVIRONMENT: env,
		},
		bindings: {
			// Per-run deploy-runner container. The RunnerContainer DO class lives in src/RunnerContainer.ts
			// and is re-exported from the worker entry (src/worker.ts) so wrangler can find it. The CF
			// container-application name is DISTINCT from the old `vozka-runner` one the control plane used
			// (CF app names are account-global + tied to a DO namespace): the old `prod-vozka-runner` app
			// stays bound to fabrika until fabrika redeploys without its Container, so this one must not clash.
			RUNNER: new Container({
				name: 'vozka-deploy-runner',
				className: 'RunnerContainer',
				// Every account builds from its pinned fabrika checkout; no cross-account image tag exists.
				image: '../runner-container/Dockerfile',
				imageBuildContext: '../..',
				maxInstances: env === 'prod' ? 10 : 3,
				instanceType,
				// Deploy containers need enough time to finish or persist terminal state before replacement.
				rolloutActiveGracePeriod: 1_200,
			}),
			// Run logs + terminal status, keyed by run id — the control plane's bucket, ADOPTED by name.
			RUN_LOGS: new R2Bucket({ name: 'vozka-run-logs' }),
			// Registry + run history — the control plane's D1, ADOPTED by name. NO migrationsDir: fabrika owns
			// the schema + migrations; vozka-runner only writes the runs row's terminal status.
			DB: new D1Database({ name: 'vozka', locationHint: 'weur' }),
		},
	})
}

export default defineApp({
	id: 'vozka-runner',
	resources: buildRunnerWorker,
	pipeline: {
		// vozka-runner's Worker source lives alongside this config. No build step (no
		// assets), and NO secrets — every credential arrives per-run in the provider job over the binding.
		workerDir: '.',
	},
})
