...
// auth, JWKS, and the `/admin/*` JSON API) together with the example proxy and app as AUXILIARY
// workers. Both run in one process against one local D1, so the example's `iam.audit()` writes land
// in the same database the Fabrika console reads.
//
	...
	workers: [
		{
			...
			config: '../../packages/provider-cloudflare/src/wrangler.jsonc',
		},
		{
