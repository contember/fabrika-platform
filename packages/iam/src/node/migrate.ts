...
//     on the same connection for the whole run — an advisory lock is per SESSION, and a pooled
//     unlock landing on a different connection would silently do nothing.
//
// Run it directly: `bun src/node/migrate.ts` (reads PROPUSTKA_DATABASE_URL).
...
/**
 * Apply every not-yet-applied migration. Returns the names applied by THIS call (empty when the
 * schema was already current). The database handle is the caller's — it must be backed by a
 * single-connection pool, or the advisory lock below does not serialize anything.
 */
...
/** CLI: apply the shipped migrations against `PROPUSTKA_DATABASE_URL`, then exit. */
async function main(): Promise<void> {
	const url = process.env['PROPUSTKA_DATABASE_URL']
	if (url === undefined || url.trim() === '') {
		throw new Error('PROPUSTKA_DATABASE_URL is required')
	}
