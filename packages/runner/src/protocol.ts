...
// control-plane Worker (`@fabrika/control`) and the in-container server (`./server.ts`) exchange.
//
 ...
 * sensitive: the container passes them to the `fabrika` child via its environment, never logs them,
...
export interface RunnerJob {
	...
	ref: string
	/** Target environment passed to `fabrika deploy --env=<env>`. */
	env: string
	/** Sub-directory within the clone the config lives in (relative to repo root). Defaults to `.`. */
	workerDir?: string
	/** Path to the config file relative to `workerDir`. Defaults to `fabrika.config.ts`. */
	configPath?: string
	/** Public domain for this stage (becomes `VOZKA_DOMAIN`), when known. */
	domain?: string
	/** Run `fabrika deploy --dry-run` (no real Cloudflare/propustka mutation). */
	dryRun?: boolean
	/**
...
export interface RunnerStatus {
	...
	state: RunnerState
	/** Process exit code of `fabrika deploy`, once the deploy phase has finished. */
	exitCode?: number
