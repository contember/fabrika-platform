...
- `src/worker.ts` — the `VozkaRunner` `WorkerEntrypoint`: `startRun(job)` = boot container + relay → R2 +
  `finishRun` → D1; `cancelRun(runId)` = destroy the run's container + `finishRun` failed (operator cancel).
  Re-exports the `RunnerContainer` DO. Default export. Imports `cloudflare:workers`.
...
the worker as a value, so importers don't pull the Workers runtime. `@fabrika/control` imports only that.
...
- **RUN_LOGS (R2) + DB (D1) are SHARED with the control plane** — oblaka ADOPTS them by remote name
  (`<env>-<name>`), so vozka-runner declares the same names + binds fabrika's existing resources. It must
  use the SAME env as the control plane, and its `DB` declares NO `migrationsDir` — fabrika owns the schema.
- **`finish-run.ts` DUPLICATES `@fabrika/control`'s `Db.markRunFinished`** (same guarded UPDATE) to avoid a
...
- **`src/index.ts` never re-exports the worker / RunnerContainer as a VALUE** — only `export type
  { VozkaRunner }` — so a plain-Bun importer of `@fabrika/runner` never loads `cloudflare:workers`.
