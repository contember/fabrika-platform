#!/usr/bin/env bun
/**
 * Deploy VOZKA-RUNNER — the deploy executor, OUT-OF-BAND. vozka-runner can't be deployed through fabrika's
 ...
 * its OWN engine on a docker host, exactly like fabrika's `scripts/bootstrap.ts` deploys fabrika — the
 ...
 *   CLOUDFLARE_ACCOUNT_ID  — the SINGLE CF account vozka-runner runs on (must match fabrika's, so the
...
async function main(): Promise<void> {
	...
	// history is preserved (see the same note in packages/control/scripts/bootstrap.ts).
