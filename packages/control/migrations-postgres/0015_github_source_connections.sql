...
ALTER TABLE vault DROP CONSTRAINT vault_scope_check;
ALTER TABLE vault ADD CONSTRAINT vault_scope_check CHECK (scope IN ('app','app-env','platform'));
...
CREATE TABLE github_source_setup_attempts (
	...
	CHECK (credential_sha256 IS NULL OR credential_sha256 ~ '^[0-9a-f]{64}$'),
