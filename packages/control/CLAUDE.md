...
run off to the vozka-runner executor (`@fabrika/runner`) over a service binding.
...
**vozka-runner** (a SEPARATE worker, `@fabrika/runner`) over the `RUNNER_SVC` service binding. vozka-runner
...
- **The deploy EXECUTOR is a separate worker (`@fabrika/runner` / `vozka-runner`), reached via `RUNNER_SVC`.**
