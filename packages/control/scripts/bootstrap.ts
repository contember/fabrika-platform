 ...
 *
 *   1. propustka FIRST. fabrika authenticates + authorizes through propustka, so propustka's admin
 *      front door must exist before fabrika can reconcile its own Access/schema into it. Deploy
 ...
 *      (reads propustka's committed `packages/iam/propustka.access.ts`; see that script's header
 *      for CF_API_TOKEN / CF_ACCOUNT_ID / PROPUSTKA_HOSTNAME / PROPUSTKA_HUMAN_EMAIL_DOMAINS). Then
 *      mint a provisioning key for fabrika (propustka `scripts/provision-key.ts`) and paste the
 *      resulting `px_` key into this script's PROPUSTKA_PROVISIONING_KEY.
 *
 *   2. fabrika SECOND — THIS script. Deploys fabrika via the engine with VOZKA_BOOTSTRAP_ADMINS set to the
 ...
 *   PROPUSTKA_URL, PROPUSTKA_PROVISIONING_KEY — the one propustka's base URL + fabrika's seeded provisioning
 *                                                    `px_` key. Become fabrika's runtime config so it
 *                                                    reconciles every app it deploys; also reconcile fabrika.
 *   VOZKA_DOMAIN                                   — fabrika's hostname (drives Access destinations + vars).
...
async function main(): Promise<void> {
	// VOZKA_DOMAIN must be set before importing fabrika.config (its `access` declaration throws at import
	...
	// var path picks up fabrika.config's `VOZKA_BOOTSTRAP_ADMINS` var, which reads process.env). We log
