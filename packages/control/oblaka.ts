...
// lives in `fabrika.config.ts` (the single source of truth, dogfooding @fabrika/config), and this file
...
// the `fabrika deploy` path; locally there is no public domain, so `buildVozkaWorker` receives just env.
//
// The off-local `fabrika deploy` self-deploy path does NOT go through this file — it loads
