...
--
-- The `transport_kind` CHECK still names `legacy-v1` on purpose: narrowing it on SQLite needs a full
-- table rebuild, and no writer can produce the value any more — `publishConnection` writes the literal
-- and `parseTransportKind` refuses it on read.
