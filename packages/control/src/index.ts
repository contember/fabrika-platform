import { isRunnerJob, type RelayResult, type RunnerJob } from '@fabrika/runner'
...
/**
 * The fabrika control-plane Worker — a single `WorkerEntrypoint` carrying:
...
export class Vozka extends WorkerEntrypoint<Env> {
	 ...
	 * is exactly what a fabrika self-deploy does: it resets fabrika, not vozka-runner). The queue consumer
	...
	/**
	 * Cancel an in-flight run: have vozka-runner DESTROY the run's container (off-local), then free the
	 * per-app-env deploy lock so the target can be redeployed immediately. Destroying the container is what
	 * makes the lock release safe — it guarantees no orphaned `wrangler deploy` races a fresh run. Locally
	 * (no vozka-runner) there is no container; just mark the run failed. The `runs.status` guard makes the
	 * terminal write idempotent, so a concurrent relay finish is a no-op.
	 */
	async cancelRun(run: RunRow): Promise<void> {
		if (this.env.RUNNER_SVC !== undefined) {
			await this.env.RUNNER_SVC.cancelRun(run.id)
		} else {
			await new Db(this.env.DB).markRunFinished(run.id, 'failed', null)
		}
		await this.env.DEPLOY_LOCK.get(this.env.DEPLOY_LOCK.idFromName(`${run.app_id}:${run.env}`)).release(run.id)
	}
	...
	override async fetch(request: Request): Promise<Response> {
		...
		if (url.pathname.startsWith('/api/')) {
			return handleApi(request, {
				...
				repoSource: this.repoSource(),
				cancelRun: (run) => this.cancelRun(run),
				vault: () => this.vault(),
			})
		}
	...
	/**
	 * The cron handler (triggers.crons in fabrika.config.ts, every 5 min): poll PUBLIC repos (apps with no
