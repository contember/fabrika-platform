 ...
 * then `can(action, scope?)`-checks against the fabrika vocabulary (src/actions.ts). Audit goes through
 * the resolved AuthContext.
 *
 * Two modes, selected by the `DEV` var (set in oblaka.ts):
 *   - local (`DEV='true'`)  → a fabrika-synthesized AuthContext from a fixed dev persona (no propustka).
...
/** fabrika's per-request authentication outcome — the dev fake and `PropustkaAuth` both resolve to this. */
...
export interface IamEnv {
	 ...
	 * escape hatch for the FIRST operator before propustka knows about fabrika (see `withBootstrapAdmins`).
	 */
 ...
 * denies, so the first operator can use fabrika before propustka knows about it. Empty list → a
 ...
 * human hatch. propustka admits this env-only key on its OWN admin endpoints (`resolveCaller`), but fabrika's
