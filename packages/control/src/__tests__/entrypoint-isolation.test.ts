...
// (`@fabrika/platform-node` is Bun-only by construction and is listed as such); their own internals are
// not this package's business.
//
// ONE THING THE SPECIFIER RULE CANNOT SEE, and which is therefore asserted by FILE instead:
// `platform-cf.ts` names `D1Database` / `R2Bucket` / `Queue` / `Service` / `Fetcher`, which are AMBIENT
// globals from `@cloudflare/workers-types` — no import, so no specifier to catch. It is Cloudflare-only
// all the same (its whole job is adapting those bindings), and the Bun graph must not reach it.
...
const CLOUDFLARE_ONLY = (specifier: string): boolean => specifier.startsWith('cloudflare:') || specifier === '@cloudflare/containers'
...
describe('entrypoint isolation', () => {
	...
	test('the Bun entrypoint reaches no Cloudflare-only module', () => {
		...
		// …and in particular it must never pull in `index.ts` (the only file importing the Worker API) or
		// `platform-cf.ts` (whose Cloudflare coupling is ambient types, invisible to the specifier rule).
		expect(graph.files).not.toContain('index.ts')
		expect(graph.files).not.toContain('platform-cf.ts')
	...
	test('the cron command and the migration runner stay on the process side too', () => {
		for (const entry of ['node/cron.ts', 'node/migrate.ts']) {
			const graph = walk(entry)
			expect([...graph.externals].filter(CLOUDFLARE_ONLY)).toEqual([])
			expect(graph.files).not.toContain('index.ts')
			expect(graph.files).not.toContain('platform-cf.ts')
		}
		// The maintenance work itself is shared — `scheduled()` on Workers calls the same function.
		expect(walk('node/cron.ts').files).toContain('cron.ts')
		expect(walk('index.ts').files).toContain('cron.ts')
	})

	test('the SHARED layer names neither runtime', () => {
		// If any of these ever needs a runtime-specific import, the dependency belongs behind a port in
		// `env.ts`, not inlined here.
		for (const entry of SHARED) {
			const graph = walk(entry)
			expect(`${entry}: ${[...graph.externals].filter(CLOUDFLARE_ONLY).join(',')}`).toBe(`${entry}: `)
			expect(`${entry}: ${[...graph.externals].filter(PROCESS_ONLY).join(',')}`).toBe(`${entry}: `)
		}
	})

	test('the shared layer never reaches the Cloudflare adapter either', () => {
		// `platform-cf.ts` is where the Cloudflare binding types are allowed to appear. Only `index.ts`
		// may import it; a shared module that did would compile on Bun and fail only at typecheck time on
