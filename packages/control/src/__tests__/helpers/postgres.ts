...
// and `@fabrika/iam` use, so one `FABRIKA_TEST_POSTGRES_URL` turns on every Postgres-backed suite in the repo.
//
// They SKIP rather than fail when it is unset: `bun test` has to be green on a laptop with no database.
// What is lost by skipping is stated plainly in the skip message, because a silently empty suite is
// worse than no suite.
//
...
// every pooled connection sees it. That gives real isolation without renaming a single table, and lets
// the SHIPPED migrations be applied verbatim, through the SHIPPED runner — the tests exercise what
// deploys, not a re-spelling of it. The URL is a credential and is never logged, not on failure.
...
export const skipReason =
	`skipped: set ${URL_VAR} to run the Postgres-backed control-plane tests (they apply migrations-postgres/ and run the db.ts + vault query surface against it)`
