// Argument parsing for the `fabrika` CLI — pure + side-effect-free, so it's unit-testable WITHOUT running
...
/**
 * The control-plane BASE components, in DEPLOY ORDER: vozka-runner FIRST, then fabrika. fabrika binds
 * `RUNNER_SVC` → vozka-runner, so the runner must already exist or `wrangler deploy` of fabrika fails to
