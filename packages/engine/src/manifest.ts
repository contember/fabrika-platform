...
import { compileImportYaml } from './drivers/zerops/compile'
...
export function configFromManifest(manifest: FabrikaManifestV1): ZeropsCompiledAppConfig {
	const pipeline: AppPipeline = {
		secrets: [...manifest.app.pipeline.secrets],
		vars: [...manifest.app.pipeline.vars],
	}
	return {
		id: manifest.app.id,
		...(manifest.app.schema !== undefined ? { schema: manifest.app.schema } : {}),
		pipeline,
		target: {
			platform: 'zerops',
			compiled: {
				importYaml: manifest.target.importYaml,
				serviceHostnames: [...manifest.target.serviceHostnames],
			},
			deployService: manifest.target.deployService,
			...(manifest.target.zeropsSetup !== undefined ? { zeropsSetup: manifest.target.zeropsSetup } : {}),
			...(manifest.target.proxy !== undefined ? { proxy: manifest.target.proxy } : {}),
		},
	}
}
