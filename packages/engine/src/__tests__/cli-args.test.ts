...
describe('parseArgs', () => {
	...
	test('deploy: config defaults to ./fabrika.config.ts', () => {
	...
	test('platform deploy: subcommand + runner/worker configs + build flag', () => {
		const a = parseArgs([
			'platform',
			'deploy',
			'--runner-config=packages/runner/fabrika-runner.config.ts',
			'--worker-config=packages/control/fabrika.config.ts',
			'--build-runner-image',
		])
		expect(a.command).toBe('platform')
		expect(a.subcommand).toBe('deploy')
		expect(a.runnerConfig).toBe('packages/runner/fabrika-runner.config.ts')
		expect(a.workerConfig).toBe('packages/control/fabrika.config.ts')
