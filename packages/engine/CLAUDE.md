...
The deploy engine + the `fabrika` CLI. Turns an `AppConfig` + `DeployContext` into an ordered plan and
...
- **oblaka state is per-app: `<app id>-state`** (overridable via `ctx.stateNamespace`). oblaka keys state by
  ...
  fabrika deploy continues its existing state instead of re-provisioning.
