...
//
// The DTO types below are NOT generated and NOT imported from `@fabrika/control` — that package's
...
// shapes here mirror the `toXDto` mappers in `@fabrika/control/src/api/{registry,runs}.ts` exactly —
// they ARE the contract this SPA consumes. The `LogLine` wire shape is likewise mirrored from
// `@fabrika/runner`'s protocol module; we don't import the runner package itself because its entry
// re-exports the in-container server engine (Bun globals), which doesn't type-check in a browser
// build. If `@fabrika/runner` ever adds a pure `./protocol` subpath export, swap this for an import.

// ── Run log lines (mirror @fabrika/runner protocol LogLine) ───────────────────────
...
export interface AppOptionalFields {
	...
	buildCmd?: string | null
	configPath?: string | null
	/** Set the installation id manually. Omit and pass `resolveInstallationId` to auto-detect instead. */
	githubInstallationId?: number | null
	/** Request-only flag: auto-detect the GitHub installation id from the repo URL (ignored if `githubInstallationId` is set). */
