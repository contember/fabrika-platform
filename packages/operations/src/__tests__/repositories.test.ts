...
describe('Operations portable repositories', () => {
	...
	test('persists triage activity and keeps merge targets inside a source', async () => {
		...
		expect((await harness.repositories.operator.listIssues({ sourceIds: ['source-a'], sort: 'recent', limit: 10 })).map((issue) => issue.fingerprint))
			.toEqual([
				'target',
			])
		expect(await harness.repositories.operator.issueStatusCounts({ sourceIds: ['source-a'] })).toEqual([{ status: 'open', count: 1 }])
