...
ALTER TABLE releases ADD COLUMN updated_at BIGINT;

UPDATE releases
SET release_name = id, updated_at = created_at
WHERE release_name IS NULL;

ALTER TABLE releases ALTER COLUMN release_name SET NOT NULL;
ALTER TABLE releases ALTER COLUMN updated_at SET NOT NULL;

CREATE UNIQUE INDEX idx_releases_source_name ON releases(source_id, release_name);
CREATE UNIQUE INDEX idx_releases_source_commit ON releases(source_id, commit_sha);
...
ALTER TABLE source_maps ADD COLUMN byte_length BIGINT;
