...
import { isCloudflareRunnerJob } from '@fabrika/provider-cloudflare/runner'
import { RUNNER_HEALTH_PATH, RUNNER_LOGS_PATH, RUNNER_PORT, RUNNER_RUN_PATH, RUNNER_STATUS_PATH } from '@fabrika/runner-contract'
...
const encodeLine = (line: RunLogLine): Uint8Array => new TextEncoder().encode(`${JSON.stringify(line)}\n`)
...
export const createServer = (env: RunnerEnv): RunnerServer => {
	...
	const handle = async (request: Request): Promise<Response> => {
		...
		if (request.method === 'POST' && url.pathname === RUNNER_RUN_PATH) {
			return handleRun(request)
		}
		if (request.method === 'GET' && url.pathname === RUNNER_LOGS_PATH) {
			return handleLogs()
		}
		if (request.method === 'GET' && url.pathname === RUNNER_STATUS_PATH) {
