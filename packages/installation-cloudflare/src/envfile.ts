/**
 * Resume support: persist captured secrets to the `.env` the CLI already reads, so an abort (or a
 * crash) at ANY point can be recovered by simply re-running — the second run picks every value back up
 * from `.env` and SKIPS re-creating the external resources (GitHub App, vault key, provisioning key)
 * that would otherwise be orphaned with their secrets lost.
 *
 * `.env` is gitignored (the scaffolded base repo ships a `.gitignore` covering it), and Bun auto-loads
 * it for `bunx`, so a persisted value is visible to the next run's `process.env` for free. Values are
 * encoded so they survive Bun's `.env` loader exactly (see `encodeEnvValue` — single-quote literal for
 * one-liners, double-quote + `\n` for the PEM). This module NEVER logs a secret value — only the `.env`
