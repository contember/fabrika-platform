...
Generated and maintained by **`fabrika init {{ACCOUNT}}`** (`@fabrika/cli`). Secrets and variables live in
...
`fabrika platform deploy` for `packages/runner` and `packages/control`. The workflow is idempotent, so
re-running it is a safe redeploy.

## First bring-up

Run `fabrika init {{ACCOUNT}}` from a laptop with the Cloudflare API token in hand. It creates this repo,
...
`fabrika.ref` is operator-owned. `fabrika init` creates it once and does not overwrite later version pins.
