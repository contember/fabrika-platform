/**
 * GitHub App creation via the MANIFEST FLOW — the CLI's centerpiece for the App. Instead of
 * hand-creating an App in the GitHub UI and copying a dozen fields, we POST a manifest to GitHub, the
 * operator clicks "Create", GitHub redirects back to a tiny local server with a one-time `code`, and we
 * exchange it for the App's id + PEM private key + webhook secret in one call. See:
 ...
 * routes them into the base-repo secrets) but never logs them. Only the App's id / slug / html_url are
...
/**
 * The GitHub App manifest. Minimal permissions for fabrika's private-repo push deploys: read `contents`
 * (clone) + the `push` event (the webhook trigger). The webhook posts to fabrika's own ingest route.
...
export async function promptInstall(app: CreatedGitHubApp, repos: string[]): Promise<void> {
	action('OPERATOR ACTION — install the GitHub App', [
		`1. Open: ${url(`https://github.com/apps/${app.slug}/installations/new`)}`,
 ...
 * the CLI), so this only succeeds if the operator's `gh` happens to have org-admin visibility;
...
<body style="font-family: system-ui; max-width: 40rem; margin: 4rem auto; text-align: center;">
	...
	<p>Return to your terminal — the CLI has captured the App credentials.</p>
</body></html>`
}
