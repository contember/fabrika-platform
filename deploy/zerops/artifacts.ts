...
const topologyArtifacts = (): Artifact[] =>
	fabrikaTopologies().flatMap((topology) => {
		...
		return [
			{
				path: `deploy/zerops/generated/${topology.id}.provision.zerops-import.yaml`,
				content: IMPORT_HEADER('topology.ts', 'POST /client/{clientId}/project/import  { yaml }') + compiled.provision.yaml,
			},
			{
				path: `deploy/zerops/generated/${topology.id}.zerops-import.yaml`,
...
/**
 * The repository-root `zerops.yaml`. One file, three named setups, selected by `zeropsSetup` (which
