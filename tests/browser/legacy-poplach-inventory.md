...
| `email-channel.test.ts`                                  | Deferred    | Fabrika Operations supports webhook channels only. Portable email delivery is tracked in [`backlog 46`](../../docs/backlog/46-add-portable-email-alert-delivery.md).                                                          |
...
| `spike-alert.test.ts`                                    | Covered     | `operations-alert-routing.test.ts` proves configuration and webhook lifecycle. `alerts.test.ts` and `consumer.test.ts` prove scheduled detection, transition production, outbox deduplication, and webhook delivery.          |
