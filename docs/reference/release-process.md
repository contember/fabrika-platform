...
`scripts/release.ts` validates these rules and computes the dependency-first order. The current set contains nineteen packages.
...
After publication, `release:registry-smoke` waits for the expected `latest` or `next` dist-tag on all nineteen packages, installs every exact version from npm into a clean project, verifies the installed manifests, imports the representative application/provider surfaces, and executes the installed CLI.
