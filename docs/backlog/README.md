...
- [52](52-revoke-sessions-an-operator-no-longer-trusts.md) — No admin surface can revoke another principal's session; only the holder can. (Dev-bypass half shipped.)
...
- [54](54-give-operations-its-own-proxy-app-identity.md) — Cloudflare and the shared proxy disagree about Operations' app id, so its gate rules would not work if they mattered.
